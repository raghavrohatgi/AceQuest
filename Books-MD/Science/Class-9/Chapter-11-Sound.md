...
chapter_number: 11
chapter_title: "Sound"
source_pdf: "CBSE Books Science/Class 9/Chapter 11 - Sound.pdf"
...
foundation for future development of radio, telephone, telegraph and even television. He also discovered the photoelectric effect which was later explained by Albert Einstein. The SI unit of frequency was named as hertz in his honour.
...
density of the medium oscillates between a maximum value and a minimum value. The change in density from the maximum value to the minimum value, then again to the maximum value, makes one complete oscillation. The number of such oscillations per unit time is the frequency of the sound wave. If we can count the number of the compressions or rarefactions that cross us per unit time, we will get the frequency of the sound wave. It is usually represented by  $\nu$  (Greek letter, nu). Its SI unit is hertz (symbol, Hz).
...
Example 11.1 A sound wave has a frequency of  $2\mathrm{kHz}$  and wave length  $35~\mathrm{cm}$ . How long will it take to travel  $1.5\mathrm{km}$ ?

# Solution:

Given,

Frequency,  $\nu = 2\mathrm{kHz} = 2000\mathrm{Hz}$
...
$$
\begin{array}{l} \nu = \lambda \nu \\ = 0. 3 5 \mathrm {m} \quad 2 0 0 0 \mathrm {H z} = 7 0 0 \mathrm {m} / \mathrm {s} \\ \end{array}
$$

The time taken by the wave to travel a distance,  $d$  of  $1.5\mathrm{km}$  is

$$
t = \frac {d}{v} = \frac {1 . 5 \times 1 0 0 0 m}{7 0 0 m s ^ {- 1}} = \frac {1 5}{7} s = 2. 1 s.
$$

Thus sound will take 2.1 s to travel a distance of  $1.5 \, \text{km}$ .
...
3. Calculate the wavelength of a sound wave whose frequency is  $220\mathrm{Hz}$  and speed is  $440\mathrm{m / s}$  in a given medium.
4. A person is listening to a tone of  $500\mathrm{Hz}$  sitting at a distance of  $450\mathrm{m}$  from the source of the sound. What is the time interval between successive compressions from the source?
...
Sound propagates through a medium at a finite speed. The sound of a thunder is heard a little later than the flash of light is seen. So, we can make out that sound travels with a speed which is much less than the speed of light. The speed of sound depends on the properties of the medium through which it travels. You will learn about this dependence in higher classes. The speed of sound in a medium depends on temperature of the medium. The speed of sound decreases when we go from solid to gaseous state. In any medium as we increase the temperature, the speed of sound increases. For example, the speed of sound in air is 331 m s $^{-1}$  at  $0^{\circ}\mathrm{C}$  and 344 m s $^{-1}$  at  $22^{\circ}\mathrm{C}$ . The speeds of sound at a particular temperature in various media are listed in Table 11.1. You need not memorise the values.
...
Example 11.2 A person clapped his hands near a cliff and heard the echo after $2\mathrm{s}$.

What is the distance of the cliff from the person if the speed of the sound, $\nu$ is taken as $346\mathrm{m}\mathrm{s}^{-1}$?

# Solution:

Given,

Speed of sound, $\nu = 346\mathrm{m}\mathrm{s}^{-1}$
...
$$
= \nu \times t = 346 \mathrm{m} \mathrm{s}^{-1} \times 2 \mathrm{s} = 692 \mathrm{m}
$$
...
13. A sound wave travels at a speed of $339\,\mathrm{m\,s^{-1}}$. If its wavelength is $1.5\,\mathrm{cm}$, what is the frequency of the wave? Will it be audible?
