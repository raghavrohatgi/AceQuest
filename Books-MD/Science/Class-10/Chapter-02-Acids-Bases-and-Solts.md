...
grade: 10
chapter_number: 2
chapter_title: "Acids, Bases and Solts"
source_pdf: "CBSE Books Science/Class 10/Chapter 2 - Acids, Bases and Solts .pdf"
...
- Collect the following solutions from the science laboratory-hydrochloric acid (HCl), sulphuric acid  $(\mathrm{H}_2\mathrm{SO}_4)$ , nitric acid  $(\mathrm{HNO}_3)$ , acetic acid  $(\mathrm{CH}_3\mathrm{COOH})$ , sodium hydroxide (NaOH), calcium hydroxide  $[\mathrm{Ca(OH)}_2]$ , potassium hydroxide (KOH), magnesium hydroxide  $[\mathrm{Mg(OH)}_2]$ , and ammonium hydroxide  $(\mathrm{NH}_4\mathrm{OH})$ .
...
$$
2 \mathrm{NaOH}(\mathrm{aq}) + \mathrm{Zn}(\mathrm{s}) \rightarrow \mathrm{Na_2ZnO_3}(\mathrm{s}) + \mathrm{H_2}(\mathrm{g})
$$
...
- Take about 0.5 g of sodium carbonate ($\mathrm{Na_2CO_3}$) in test tube A and about 0.5 g of sodium hydrogencarbonate ($\mathrm{NaHCO_3}$) in test tube B.
...
Test tube A: $\mathrm{Na_2CO_3(s)} + 2\mathrm{HCl(aq)} \rightarrow 2\mathrm{NaCl(aq)} + \mathrm{H_2O(l)} + \mathrm{CO_2(g)}$

Test tube B: $\mathrm{NaHCO_3(s)} + \mathrm{HCl(aq)} \rightarrow \mathrm{NaCl(aq)} + \mathrm{H_2O(l)} + \mathrm{CO_2(g)}$

On passing the carbon dioxide gas evolved through lime water,

$$
\mathrm{Ca(OH)_2(aq)} + \mathrm{CO_2(g)} \rightarrow \mathrm{CaCO_3(s)} + \mathrm{H_2O(l)}
$$
...
$$
\mathrm{CaCO_3(s)} + \mathrm{H_2O(l)} + \mathrm{CO_2(g)} \rightarrow \mathrm{Ca(HCO_3)_2(aq)}
$$
...
$$
\mathrm{Base} + \mathrm{Acid} \rightarrow \mathrm{Salt} + \mathrm{Water}
$$
...
$$
\mathrm{Metal\ oxide} + \mathrm{Acid} \rightarrow \mathrm{Salt} + \mathrm{Water}
$$
...
Acids contain  $\mathrm{H^{+}}$  ion as cation and anion such as  $\mathrm{Cl^-}$  in  $\mathrm{HCl}$ ,  $\mathrm{NO}_3^-$  in  $\mathrm{HNO}_3$ ,  $\mathrm{SO}_4^{2-}$  in  $\mathrm{H}_2\mathrm{SO}_4$ ,  $\mathrm{CH}_2\mathrm{COO}^-$  in  $\mathrm{CH}_2\mathrm{COOH}$ . Since the cation present in acids is  $\mathrm{H^{+}}$ , this suggests that acids produce hydrogen ions,  $\mathrm{H^{+}(aq)}$ , in solution, which are responsible for their acidic properties.
...
Hydrogen ions cannot exist alone, but they exist after combining with water molecules. Thus hydrogen ions must always be shown as  $\mathrm{H^{+}(aq)}$  or hydronium ion  $(\mathrm{H}_{3}\mathrm{O}^{+})$ .

$$
\mathrm {H} ^ {+} + \mathrm {H} _ {2} \mathrm {O} \rightarrow \mathrm {H} _ {3} \mathrm {O} ^ {+}
$$

We have seen that acids give  $\mathrm{H}_3\mathrm{O}^+$  or  $\mathrm{H^{+}(aq)}$  ion in water. Let us see what happens when a base is dissolved in water.

$$
\mathrm {N a O H (s)} \xrightarrow {\mathrm {H} _ {2} \mathrm {O}} \mathrm {N a} ^ {+} (\mathrm {a q}) + \mathrm {O H} ^ {-} (\mathrm {a q})
$$
...
$$
\mathrm{KOH}(\mathrm{s}) \xrightarrow{\mathrm{H}_2\mathrm{O}} \mathrm{K}^+(\mathrm{aq}) + \mathrm{OH}^-(\mathrm{aq})
$$

$$
\mathrm{Mg}(\mathrm{OH})_2(\mathrm{s}) \xrightarrow{\mathrm{H}_2\mathrm{O}} \mathrm{Mg}^{2+}(\mathrm{aq}) + 2\mathrm{OH}^-(\mathrm{aq})
$$
...
$$
\mathrm{H} \xrightarrow{\mathrm{X} + \mathrm{M}} \mathrm{OH} \rightarrow \mathrm{MX} + \mathrm{HOH}
$$
...
- Add a few drops of concentrated $\mathrm{H}_2\mathrm{SO}_4$ to it and swirl the beaker slowly.
...
Mixing an acid or base with water results in decrease in the concentration of ions $(\mathrm{H}_3\mathrm{O}^+/\mathrm{OH}^-)$ per unit volume. Such a process is called dilution and the acid or the base is said to be diluted.
...
1. Why do HCl, HNO $_2$ , etc., show acidic characters in aqueous solutions while solutions of compounds like alcohol and glucose do not show acidic character?
...
5. How is the concentration of hydronium ions  $(\mathrm{H}_2\mathrm{O}^+)$  affected when a solution of an acid is diluted?
...
Figure 2.6 Variation of pH with the change in concentration of  $\mathrm{H^{+}(aq)}$  and  $\mathrm{OH^{-}(aq)}$  ions
...
|  Activity 2.11 | S. No. | Solution | Colour of pH paper | Approx-imate pH value | Nature of substance  |
| --- | --- | --- | --- | --- | --- |
|  Test the pH values of solutions given in Table 2.2. | 1 | Saliva (before meal) |  |  |   |
|  Record your observations. | 2 | Saliva (after meal) |  |  |   |
|  What is the nature of each substance on the basis of your observations? | 3 | Lemon juice |  |  |   |
...
$$
2 \mathrm{NaCl}(\mathrm{aq}) + 2 \mathrm{H_2O}(\mathrm{l}) \rightarrow 2 \mathrm{NaOH}(\mathrm{aq}) + \mathrm{Cl_2}(\mathrm{g}) + \mathrm{H_2}(\mathrm{g})
$$
...
$$
2 \mathrm{Ca}(\mathrm{OH})_{2} + 2 \mathrm{Cl}_{2} \rightarrow \mathrm{Ca}(\mathrm{ClO})_{2} + \mathrm{CaCl}_{2} + 2 \mathrm{H_2O}
$$
...
The baking soda is commonly used in the kitchen for making tasty crispy pakoras, etc. Sometimes it is added for faster cooking. The chemical name of the compound is sodium hydrogencarbonate (NaHCO₃). It is produced using sodium chloride as one of the raw materials.

$$
\mathrm{NaCl} + \mathrm{H_2O} + \mathrm{CO_2} + \mathrm{NH_3} \rightarrow \mathrm{NH_4Cl} + \mathrm{NaHCO_3}
$$
...
$$
2\mathrm{NaHCO_3} \xrightarrow{\text{Heat}} \mathrm{Na_2CO_3} + \mathrm{H_2O} + \mathrm{CO_2}
$$
...
$$
\mathrm{NaHCO_3} + \mathrm{H^+} \rightarrow \mathrm{CO_2} + \mathrm{H_2O} + \text{Sodium salt of acid}
$$
(From any acid)
...
Another chemical that can be obtained from sodium chloride is Na₂CO₃·10H₂O (washing soda). You have seen above that sodium carbonate can be obtained by heating baking soda; recrystallisation of sodium carbonate gives washing soda. It is also a basic salt.

$$
\mathrm{Na_2CO_3} + 10\mathrm{H_2O} \rightarrow \mathrm{Na_2CO_3} \cdot 10\mathrm{H_2O}
$$
(Sodium carbonate)

What does 10H₂O signify? Does it make Na₂CO₃ wet? We will address this question in the next section.
...
Water of crystallisation is the fixed number of water molecules present in one formula unit of a salt. Five water molecules are present in one formula unit of copper sulphate. Chemical formula for hydrated copper sulphate is CuSO₄·5H₂O. Now you would be able to answer the question whether the molecule of Na₂CO₃·10H₂O is wet.

One other salt, which possesses water of crystallisation is gypsum. It has two water molecules as water of crystallisation. It has the chemical formula CaSO₄·2H₂O. Let us look into the use of this salt.

## Plaster of Paris

On heating gypsum at 373 K, it loses water molecules and becomes calcium sulphate hemihydrate (CaSO₄·$\frac{1}{2}$ H₂O). This is called Plaster of
...
$$
\mathrm{CaSO_4} \cdot \frac{1}{2} \mathrm{H_2O} + 1 \frac{1}{2} \mathrm{H_2O} \rightarrow \mathrm{CaSO_4} \cdot 2 \mathrm{H_2O}
$$

(Plaster of Paris) (Gypsum)

Note that only half a water molecule is shown to be attached as water of crystallisation. How can you get half a water molecule? It is written in this form because two formula units of $\mathrm{CaSO_4}$ share one molecule of water. Plaster of Paris is used for making toys, materials for decoration and for making surfaces smooth. Try to find out why is calcium sulphate hemihydrate called 'Plaster of Paris'?
...
- Acidic nature of a substance is due to the formation of $\mathrm{H^{+}(aq)}$ ions in solution. Formation of $\mathrm{OH^{-}(aq)}$ ions in solution is responsible for the basic nature of a substance.
